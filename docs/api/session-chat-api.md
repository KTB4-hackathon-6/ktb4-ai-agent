...
문서 작성 단계에서는 `POST /api/sessions/{sessionId}/documents`를 호출한다. Spring은 같은
세션 ID로 FastAPI `POST /docs`를 호출하고, 반환된 `DocumentDraft.data`를 HWPX 템플릿에
채운다. 응답은 공통 봉투 안에 현재 초안과 `GeneratedDocument`를 함께 담으며 `bytes`는
Base64 문자열이다. 초안이 `NEEDS_INPUT`이어도 현재까지 확인된 값은 채우고 나머지는 빈칸으로
둔 HWPX를 반환한다. 프런트는 누락 질문에 답할 때 같은 경로를 반복 호출한다.
...
- FastAPI의 `/docs` 라우트와 세션 문맥 저장이 준비되어 있어야 문서 작성 공개 API를 실제로
  사용할 수 있다. Spring과 프런트의 HWPX 생성·다운로드 연결은 구현되어 있다.
