...
근로계약서와 급여명세서는 서버로 전송하기 전에 브라우저에서 페이지 이미지로 변환됩니다.
사용자가 각 페이지에서 이름·번호·개인 주소·계좌번호·서명·도장 등 민감정보 영역을 직접
드래그해 가리고, 모든 페이지를 확인해야 기존 분석 API 호출이 시작됩니다. 서버에는 원본
파일명이나 PDF가 아닌 `document-page-{번호}.jpg` 형식의 익명화된 가공본만 전송됩니다.

- PDF 페이지 렌더링에는 고정 버전의 `pdfjs-dist`를 사용하며 Worker는 프론트엔드 빌드 결과에 포함됩니다.
- 브라우저 OCR이나 자동 민감정보 탐지는 제공하지 않습니다.
- 사용자가 직접 지정한 영역만 불투명 검은색 픽셀로 가공본에 반영됩니다.
- 가공본 총합이 10MB를 넘으면 자동 전송하지 않고 문서를 나누어 다시 선택하도록 안내합니다.
- 원본과 가공 중인 문서는 브라우저 저장소에 기록하지 않고 현재 화면의 메모리에만 유지합니다.
